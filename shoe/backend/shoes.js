...
app.get("/api/shoes", async (req, res) => {
  try {
    ...
    res.json(response.data); // Return the API data
  } catch (error) {
...
app.get("/api/shoes/:id", async (req, res) => {
  try {
    ...
    const options = {
      method: "GET",
      url: `https://shoes-collections.p.rapidapi.com/shoes/${id}`, // ✅ fixed template string
      headers: {
        "x-rapidapi-key": process.env.RAPIDAPI_KEY,
        "x-rapidapi-host": "shoes-collections.p.rapidapi.com",
      },
    ...
    console.log(`Fetching shoe with ID: ${id}`); // ✅ fixed quotes
    const response = await axios.request(options);
    console.log("✅ Successfully fetched single shoe data");

    res.json(response.data);
  } catch (error) {
    console.error(`❌ Error fetching shoe ${req.params.id}:`, error.message); // ✅ fixed quotes
    ...
    res.status(500).json({
      message: `Error fetching shoe with ID ${req.params.id}`, // ✅ fixed quotes
      error: error.message,
    });
  }
});
